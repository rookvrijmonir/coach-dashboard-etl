...
# Lead status buckets (komt uit de ETL in hubspot_export_raw.csv)
# status_bucket ∈ {actief, gewonnen, verloren}
...
if hs_df is not None and pv_df is not None:
    ...
    # --- DATA FILTERING ---
    # Tijd & Status filter (op status_bucket uit de ETL)
    bucket_list = []
    if show_actief:
        bucket_list.append('actief')
    if show_gewonnen:
        bucket_list.append('gewonnen')
    if show_verloren:
        bucket_list.append('verloren')

    if bucket_list:
        hs_f = hs_df[
            (hs_df['createdate'].dt.date >= start_d)
            & (hs_df['createdate'].dt.date <= end_d)
            & (hs_df['status_bucket'].astype(str).isin(bucket_list))
        ].copy()
    else:
        hs_f = hs_df.iloc[0:0].copy()
    ...
    def get_map_category(row):
        if selected_coach == "TOTAAL OVERZICHT":
            return "Totaal"
        if str(row['Coach']) == selected_coach:
            return "Focus"
        if str(row['BonusCoach']) == selected_coach:
            return "Bonus"
        if str(row['Resterende Coach']) == selected_coach:
            return "Resterend"
        if str(row['Overnemende Coach']) == selected_coach:
            return "Overname"
        return "Overig"
    ...
    if selected_coach == "TOTAAL OVERZICHT":
        ...
        with st.container():
            ...
            if not focus_df.empty:
                fig_f = px.bar(
                    focus_df, x='Leads', y='Label', orientation='h',
        ...
        with st.container():
            ...
            if not bonus_df.empty:
                fig_b = px.bar(
                    bonus_df, x='Leads', y='Label', orientation='h',
        ...
        with st.container():
            ...
            if not resterend_df.empty:
                fig_r = px.bar(
                    resterend_df, x='Leads', y='Label', orientation='h',
        ...
        fig_coach = px.bar(
            df_coach, x='Leads', y='Postcode_Match', color='KaartCategorie', barmode='group',
